      ...
      `}
        ...
        >
          <div
            className={cn(
              className,
              "spinner rounded-full bg-[conic-gradient(from_0deg_at_50%_50%,rgb(42,67,233)_0deg,rgba(42,138,246,0)_360deg)]",
            )}
          />
        </div>
      </div>
...
const StatusBorder = ({
  ...
  return (
    <>
      <div
        className={cn(
          "absolute -top-[1.2px] -left-[1.2px] h-[calc(100%+2px)] w-[calc(100%+2px)] rounded-[9px] border-2",
          className,
        )}
      />
      {children}
...
export const NodeStatusIndicator = ({
  ...
  switch (status) {
    ...
    case "success":
      return (
        <StatusBorder className={cn(className, "border-green-400")}>
          {children}
        </StatusBorder>
      );
    case "error":
      return (
        <StatusBorder className={cn(className, "border-red-400")}>
          {children}
        </StatusBorder>
      );
    default:
